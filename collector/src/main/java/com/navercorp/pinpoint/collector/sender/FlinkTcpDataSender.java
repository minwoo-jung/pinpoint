...
public class FlinkTcpDataSender extends TcpDataSender {
    ...
    public FlinkTcpDataSender(String name, String host, int port, PinpointClientFactory clientFactory, FlinkHeaderTBaseSerializer serializer) {
    ...
    public void send(FlinkRequest flinkRequest) {
        serialize에서 locator 이용해서 header 만들어서 호출해준다.
        executor.execute(flinkRequest);
    }
    ...
    protected void sendPacket(Object flinkRequest) {
        try {
            if (flinkRequest instanceof FlinkRequest) {
                byte[] copy = serialize((FlinkRequest)flinkRequest);
                if (copy == null) {
                    return;
                }
                doSend(copy);
    ...
    protected byte[] serialize(FlinkRequest flinkRequest) throws TException {
        return flinkHeaderTBaseSerializer.serialize(flinkRequest);
    }
}
