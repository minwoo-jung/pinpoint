...
export class CallTreeComponent implements OnInit, OnChanges, AfterViewInit {
    ...
    ngOnInit() {
        ...
        this.webAppSettingDataService.getSecurityGuideUrl().subscribe((url: string) => {
            this.securityGuideUrl = url;
        });
    }
    ...
    private makeColumnDefs(): any {
        return [
            ...
            },
            {
                ...
                cellRenderer: (params: any) => {
                    if (params.data.isAuthorized === false) {
                        return this.getAuthorizedText(params.data.argument);
                    } else {
    ...
    getAuthorizedText(text: string): HTMLElement {
        const aTag = this.windowRefService.nativeWindow.document.createElement('a');
        const iTag = this.windowRefService.nativeWindow.document.createElement('i');
        const spanTag = this.windowRefService.nativeWindow.document.createElement('span');
        iTag.className = 'fas fa-external-link-alt fa-xs';
        spanTag.textContent = ' ' + text;
        aTag.style.color = '#AAA';
        aTag.setAttribute('href', this.securityGuideUrl);
    ...
    innerCellRenderer(params: any) {
        ...
        if (params.data.isAuthorized === false) {
            return '<span style="color:#AAA;margin-left:3px"><a href="' + this.securityGuideUrl + '" target="_blank"><i class="fas fa-external-link-alt fa-xs"></i></a>' + result + params.data.method + '</span>';
        } else {
