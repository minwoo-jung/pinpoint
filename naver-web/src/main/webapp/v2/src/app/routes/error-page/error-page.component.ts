...
export class ErrorPageComponent implements OnInit {
    stateList = ['serverTime', 'configuration', 'userConfiguration', 'applicationList'];
    state: any = {
        serverTime: {
    ...
    ngOnInit() {
        ...
        this.checkUserConfiguration(this.stateList[2]);
        this.checkApplicationList(this.stateList[3]);
    }
    ...
    private checkUserConfiguration(type: string): void {
        this.userConfigurationDataService.getUserConfiguration().subscribe((configuration: IUserConfiguration) => {
            this.setState(type, true, false);
        }, (error: IServerErrorFormat) => {
            this.setState(type, false, false, error.exception.message);
        });
