 ...
 * @author minwoo.jung
 */
public class NssAuthenticationFilterTest {
    ...
    public void doFilterInternalTest() throws ServletException, IOException {
        ...
        NssAuthenticationFilter filter = new NssAuthenticationFilter();

        UserInformationAcquirer userInformationAcquirer = mock(UserInformationAcquirer.class);
        when(userInformationAcquirer.acquireUserId(request)).thenReturn(userId);
        when(userInformationAcquirer.acquireOrganizationName(request)).thenReturn(organizationName);
